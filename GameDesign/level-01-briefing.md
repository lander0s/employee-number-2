...
**Companion doc:** [game-design-document.md](game-design-document.md)
...
### What it must NOT do
- No loops. `REPEAT` is level 2's entire lesson and must not appear in the tray.
- No conditions, no pallets, no weights, no `CLOCK OUT`.
- **No pallets on the floor at all** — an empty middle floor keeps the first Floor View
  readable and gives level 4 something new to reveal.
...
|---|---|---|
| `s1` *(the only one)* | `BLUE` `BLUE` `BLUE` | 3 |

`parShipment: s1` · `singleShipment: true`

### Why exactly one

With `TAKE` and `SHIP` as the entire tray and no loop construct, **a variable-length intake is
unsolvable.** A six-instruction program can move exactly three packages; if the set contained
a four-package shipment, no legal level-1 program could clear it. Variation requires the
player to own a command that responds to variation, and that command is `REPEAT` — which is
...
Two deliberate choices about the shipment's contents:

- **All three are the same type.** A mixed intake would silently suggest that type matters,
  and type-checking is level 3. Identical boxes make "all of them" unambiguous.
- **Weights are not rendered in Act 1.** The box face is blank. Numbers appear in Act 2 with
  `MERGE WITH` (§6.2), and a player who never sees a number in the first hour never forms
  the belief that this is a math game.
...
Because all three packages are identical, this level cannot detect a wrong *order* — only a
wrong *count*. That's fine and intentional: order-sensitivity is first tested in level 3,
where the types differ. Noted here so nobody reads this level's validator as complete
coverage.
...
| Program runs past its last instruction | **The shift ends, successfully**, and the goal is checked. This is why `CLOCK OUT` isn't needed yet. |
...
| `TAKE` with claws full | **The held package is discarded** — tossed into the recycling bin, one step, no failure. The discard rule, now specified in §6.3. |

**Tray contents:** `TAKE`, `SHIP`. Two buttons. Nothing else.
...
|---|---|---|---|---|---|
| — | *(start)* | idle | empty | B B B | — |
| 1 | `TAKE` | walk to chute, catch | `BLUE` | B B | — |
| 2 | `SHIP` | walk to belt, toss | empty | B B | B |
| 3 | `TAKE` | walk to chute, catch | `BLUE` | B | B |
| 4 | `SHIP` | walk to belt, toss | empty | B | B B |
| 5 | `TAKE` | walk to chute, catch | `BLUE` | — | B B |
| 6 | `SHIP` | walk to belt, toss | empty | — | B B B |
| — | *(end of program)* | wave, screen dims | empty | — | **B B B ✓** |
...
| 6 | `TAKE TAKE SHIP TAKE SHIP` | FAIL | *"UNIT-02 shipped 2 packages. Brent asked for 3."* (first package was discarded — tests the §5 discard ruling) |
...
Case 10 is the one real playtest prediction: a meaningful share of first-timers will tap
`SHIP` first. The message must point at instruction 1, highlight it, and drop the player
straight back into editing with the caret on that row. **That single interaction is the most
important error-handling moment in the game** — it's where a player decides whether this game
makes them feel stupid.

No infinite-loop guard is reachable here: there is no loop construct in the tray.
...
- `goal.outbound` is written as the **derived** expression `shipment.intake` — "ship whatever
  arrived, in order" — not as a literal three-box list. It happens to evaluate to the same
...
│                           │
├───────────────────────────┤
│  ⊞   [ TAKE ]  [ SHIP ]   │   tray — two buttons, thumb zone
├───────────────────────────┤
│      ▶  RUN SHIFT         │
└───────────────────────────┘
```
...
1. On entering the editor, the two tray buttons pulse once, in sequence. Nothing else moves.
2. After the first insertion, the pulse stops permanently and the caret gets one bounce so
   the player sees where things land.
3. `RUN SHIFT` is dimmed while the program is empty — not disabled-with-a-scold, just quiet.
...
5. Swipe-to-Floor-View is *not* taught. Players find it during the run, or during level 2,
   and criterion 4 of the slice test measures whether they do.
...
3. Median time from end-of-call to first `RUN SHIFT` tap is **under 45 seconds**.
...
1. **`BLUE` as the first package type.** Placeholder. The first type the player ever sees
   should be the most distinctive silhouette in the set, and that's an art call once the
   package shapes exist (§16.1).
...
- *Full-claws `TAKE`* → discards the held package. Now specified as the general **discard
  rule** in §6.3, covering `PICK FROM` as well.
